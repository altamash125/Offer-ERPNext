frappe.provide("offer_customizations.customizations.timesheet");

offer_customizations.customizations.timesheet.timer = function (frm, row, timestamp = 0) {
	let dialog = new frappe.ui.Dialog({
		...
		fields: [
			...
			},
			{
				...
				onchange: function () {
					if (dialog.fields_dict.task.value) {
						frappe.db.get_value(
							"Task",
							dialog.fields_dict.task.value,
							["project", "expected_time"],
							(r) => {
								dialog.set_values({
									project: r.project,
									expected_hours: r.expected_time,
								});
							},
						);
					}
			...
			},
			{
				...
				get_query: function () {
					var p_list = [];
					for (var i = 0; i < frm.doc.projects.length; i++) {
						p_list.push(frm.doc.projects[i].project);
					}
					return {
	...
	frappe.call({
		method: "offer_customizations.customizations.timesheet.get_assigned_meeting",
	...
	offer_customizations.customizations.timesheet.control_timer(frm, dialog, row, timestamp);
	dialog.show();
};

offer_customizations.customizations.timesheet.control_timer = function (
	frm,
	dialog,
	row,
	timestamp = 0,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (!r.exc) {
						...
						frappe.show_alert({
							message: __("Task marked as Working"),
							indicator: "green",
						});
					}
				},
